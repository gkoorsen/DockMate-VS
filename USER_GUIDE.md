...
1. RDKit parses the SMILES and keeps the largest disconnected fragment. The
   **Ligand charge handling** setting defaults to **Preserve input charges**,
   retaining the fragment's formal charges. **Neutralize removable charges**
   applies RDKit's Uncharger, reproducing the legacy preparation policy. Salt
   counterions can still be removed in either mode.
2. RDKit enumerates tautomers and retains at most **Max tautomers** after a
   ...
   In preserve mode, tautomers that change atom-level formal charges are excluded.
   To retain a curated input tautomer as well as its charges, set **Max tautomers**
   to `1`.
...
state does not establish that it is appropriate at the assay pH. Curate input
charge states externally when ionization is important, choose **Preserve input
charges**, and inspect the saved `ligand_variants/*.pdbqt` files.

Headless campaign files use `single.charge_handling: preserve` (default) or
`single.charge_handling: neutralize`. The Python preparation API exposes the
same `charge_handling` field on `LigandPreparationConfig`. Both modes still add
hydrogens and assign partial charges during preparation;
formal charge handling is distinct from the partial-charge model.

Manifests record the selected policy, and ligand cache keys include it. Saved
manifests without this field are treated as legacy `neutralize` campaigns.
Changing charge handling requires a new campaign directory; selecting
`neutralize` allows otherwise-compatible legacy campaigns to resume. The
bundled ACES reproduction configurations explicitly retain `neutralize`.
Existing ESR1 results were generated with the historical preparation method
and are not regenerated by this change.
...
state-enumeration policy, charge handling, cache schema, RDKit version, and Open Babel executable
