...
class RedockAnalysisApp(tk.Tk):
    ...
    def __init__(
        ...
        self.variant_mode_var = tk.StringVar(value="best")
    ...
    def _build_ui(self) -> None:
        ...
        variant_best = tk.Radiobutton(
            variant_frame,
            text="Best protomer/tautomer only (lowest energy)",
            variable=self.variant_mode_var,
            value="best"
        )
        variant_best.grid(row=0, column=0, sticky="w")
        self._register_busy_widget(variant_best)
        variant_all_rmsd = tk.Radiobutton(
            variant_frame,
            text="Dock all variants, keep best RMSD",
            variable=self.variant_mode_var,
            value="all_rmsd"
        )
        variant_all_rmsd.grid(row=1, column=0, sticky="w")
        self._register_busy_widget(variant_all_rmsd)
        variant_all_score = tk.Radiobutton(
            variant_frame,
            text="Dock all variants, keep best score",
            variable=self.variant_mode_var,
            value="all_score"
        )
        variant_all_score.grid(row=2, column=0, sticky="w")
        self._register_busy_widget(variant_all_score)

        tk.Label(variant_frame, text="Max tautomers:").grid(row=0, column=1, sticky="e", padx=(20, 5))
        max_taut_entry = tk.Entry(variant_frame, textvariable=self.max_tautomers_var, width=6)
        max_taut_entry.grid(row=0, column=2, sticky="w")
        self._register_busy_widget(max_taut_entry)

        tk.Label(variant_frame, text="Max conformers:").grid(row=1, column=1, sticky="e", padx=(20, 5))
        max_conf_entry = tk.Entry(variant_frame, textvariable=self.max_conformers_var, width=6)
        max_conf_entry.grid(row=1, column=2, sticky="w")
    ...
    def _start_run(self) -> None:
        ...
        def _load_pairs_worker():
            try:
                ...
                message = str(exc)
                self.after(0, lambda msg=message: self._start_run_failed("Excel error", msg))
                return

            if not pairs:
                self.after(0, lambda: self._start_run_failed("No pairs", "No valid PDB/ligand pairs found."))
                return
            ...
            self.after(0, lambda: self._begin_run(pairs, col_info, config))
    ...
    def _collect_single_config(self) -> Optional[dict]:
        ...
        if not (1 <= max_tautomers <= 20):
            ...
            return None
        if not (1 <= max_conformers <= 10):
            messagebox.showerror("Input error", "Max conformers must be between 1 and 10.")
            return None
        engine = self.engine_var.get()
        logger.info(
    ...
    def _populate_empty_results(self) -> None:
        ...
        tk.Label(
            ...
            text="No results yet. Run an analysis to populate this view.",
            fg="#555555"
        ...
        for residue in receptor.get_residues():
            ...
            for atom in residue.get_atoms():
                ...
                if min_dist is None or local_min < min_dist:
                    min_dist = local_min
                    min_idx = int(heavy_indices[int(distances.argmin())])
    ...
    def _prepare_pymol_overlay(
        ...
        pose_indices = []
        for idx in (best_rmsd_idx, best_score_idx):
            if idx is None:
                continue
            if idx not in pose_indices:
                pose_indices.append(idx)

        pose_files = []
        for idx in pose_indices:
            label = "best_rmsd" if idx == best_rmsd_idx else "best_score"
    ...
    def _show_pose_viewer(self, results_path: Path) -> None:
        ...
        def _render_case(idx: int) -> None:
            ...
            def _run():
                try:
                    ...
                    payload = None
                self.after(0, lambda: _apply(payload))
        ...
        def _open_pymol():
            ...
            def _run():
                result = _worker()
                self.after(0, lambda: _apply(result))
    ...
    def _build_summary(self, results: List[RedockResult], threshold: float) -> dict:
        ...
        best_rmsds = [r.best_rmsd for r in results if r.best_rmsd is not None and r.best_rmsd < 900]
        top1_rmsds = [r.top1_rmsd for r in results if r.top1_rmsd is not None]
        runtimes = [r.runtime_sec for r in results if r.runtime_sec is not None]
        rmsd_best_scores = [r.rmsd_best_score for r in results if r.rmsd_best_score is not None]
        near_native = [r.near_native_fraction for r in results if r.near_native_fraction is not None]
        pose_counts = [r.pose_count for r in results if r.pose_count is not None]
        pearsons = [r.score_rmsd_pearson for r in results if r.score_rmsd_pearson is not None]
        spearmans = [r.score_rmsd_spearman for r in results if r.score_rmsd_spearman is not None]

        summary["success_rate_best"] = 100.0 * sum(r.best_rmsd < threshold for r in results) / len(results)
        if top1_rmsds:
            summary["success_rate_top1"] = 100.0 * sum(r.top1_rmsd < threshold for r in results if r.top1_rmsd is not None) / len(top1_rmsds)

        top5 = [r.top5_rmsd for r in results if r.top5_rmsd is not None]
        if top5:
            summary["success_rate_top5"] = 100.0 * sum(val < threshold for val in top5) / len(top5)

        top10 = [r.top10_rmsd for r in results if r.top10_rmsd is not None]
        if top10:
            summary["success_rate_top10"] = 100.0 * sum(val < threshold for val in top10) / len(top10)

        if best_rmsds:
            summary["mean_best_rmsd"] = float(np.mean(best_rmsds))
            summary["median_best_rmsd"] = float(np.median(best_rmsds))
        if top1_rmsds:
            summary["mean_top1_rmsd"] = float(np.mean(top1_rmsds))
            summary["median_top1_rmsd"] = float(np.median(top1_rmsds))
        if rmsd_best_scores:
            summary["mean_rmsd_best_score"] = float(np.mean(rmsd_best_scores))
        if near_native:
            summary["mean_near_native_fraction"] = float(np.mean(near_native))
        if pose_counts:
            summary["mean_pose_count"] = float(np.mean(pose_counts))
        if pearsons:
            summary["mean_score_rmsd_pearson"] = float(np.mean(pearsons))
        if spearmans:
            summary["mean_score_rmsd_spearman"] = float(np.mean(spearmans))
        if runtimes:
            summary["mean_runtime_sec"] = float(np.mean(runtimes))
            summary["median_runtime_sec"] = float(np.median(runtimes))

        rescore_methods = sorted(
            {r.rescore_method for r in results if r.rescore_method}
        )
        if rescore_methods:
            summary["rescore_methods"] = rescore_methods
            summary["rescore_count"] = sum(1 for r in results if r.rescore_method)
            summary["rescore_failures"] = sum(1 for r in results if r.rescore_error)

        score_labels = []
        for result in results:
            if result.control_label is None:
                continue
            rank_score = self._rank_score_value(result)
            if rank_score is None:
                continue
            label = 1 if int(result.control_label) == 1 else 0
            score_labels.append((rank_score, label))

        metrics = self._compute_enrichment_metrics(score_labels)
        if metrics:
            summary["control_actives"] = metrics["actives"]
            summary["control_decoys"] = metrics["decoys"]
            summary["roc_auc"] = metrics["roc_auc"]
            summary["log_auc"] = metrics["log_auc"]

        charges = [r.ligand_charge for r in results if r.ligand_charge is not None]
        if charges:
            summary["charge_count"] = len(charges)
            summary["mean_charge"] = float(np.mean(charges))
            summary["median_charge"] = float(np.median(charges))
            pos = sum(1 for c in charges if c > 0)
            neg = sum(1 for c in charges if c < 0)
            neu = sum(1 for c in charges if c == 0)
            total = len(charges)
            summary["charge_frac_positive"] = 100.0 * pos / total
            summary["charge_frac_negative"] = 100.0 * neg / total
            summary["charge_frac_neutral"] = 100.0 * neu / total

        score_charge_pairs = []
        for result in results:
            rank_score = self._rank_score_value(result)
            if rank_score is None or result.ligand_charge is None:
                continue
            score_charge_pairs.append((rank_score, result.ligand_charge))
        if score_charge_pairs:
            scores = [p[0] for p in score_charge_pairs]
            charges_for_scores = [p[1] for p in score_charge_pairs]
            pearson, spearman = self._score_charge_corr(scores, charges_for_scores)
            summary["score_charge_pearson"] = pearson
            summary["score_charge_spearman"] = spearman

            top_n = max(1, int(np.ceil(0.1 * len(score_charge_pairs))))
            top = sorted(score_charge_pairs, key=lambda item: item[0])[:top_n]
            top_charges = [p[1] for p in top]
            top_pos = sum(1 for c in top_charges if c > 0)
            top_neg = sum(1 for c in top_charges if c < 0)
            top_neu = sum(1 for c in top_charges if c == 0)
            summary["charge_top10_frac_positive"] = 100.0 * top_pos / len(top_charges)
            summary["charge_top10_frac_negative"] = 100.0 * top_neg / len(top_charges)
            summary["charge_top10_frac_neutral"] = 100.0 * top_neu / len(top_charges)

        by_protocol = {}
        for result in results:
            proto = result.protocol or "N/A"
            stats = by_protocol.setdefault(proto, {"count": 0, "success": 0, "mean_rmsd": []})
            stats["count"] += 1
            if result.best_rmsd < threshold:
                stats["success"] += 1
            if result.best_rmsd is not None and result.best_rmsd < 900:
        ...
        for proto, stats in by_protocol.items():
            ...
            summary["by_protocol"][proto] = stats

        by_engine = {}
        for result in results:
            eng = result.engine or "N/A"
            stats = by_engine.setdefault(eng, {"count": 0, "success": 0, "mean_rmsd": []})
            stats["count"] += 1
            if result.best_rmsd < threshold:
                stats["success"] += 1
            if result.best_rmsd is not None and result.best_rmsd < 900:
        ...
        for eng, stats in by_engine.items():
            rmsds = stats.pop("mean_rmsd")
            stats["mean_rmsd"] = float(np.mean(rmsds)) if rmsds else None
            summary["by_engine"][eng] = stats

        attempts_by_protocol = {}
        for result in results:
            if not result.protocol_attempts:
                continue
            for attempt in result.protocol_attempts:
                name = attempt.get("protocol", "N/A")
                stats = attempts_by_protocol.setdefault(name, {"attempts": 0, "success": 0, "mean_rmsd": []})
                stats["attempts"] += 1
                rmsd = attempt.get("rmsd")
                if rmsd is not None and rmsd < 900:
                    stats["mean_rmsd"].append(rmsd)
                if attempt.get("success"):
                    stats["success"] += 1
        for name, stats in attempts_by_protocol.items():
            rmsds = stats.pop("mean_rmsd")
            stats["mean_rmsd"] = float(np.mean(rmsds)) if rmsds else None
            summary["attempts_by_protocol"][name] = stats
    ...
    def _load_pairs_from_excel(
        ...
        for _, row in df.iterrows():
            ...
            if decoy_smiles_col and label_col is None:
                ...
                decoy_smiles = row.get(decoy_smiles_col)
                decoy_text = None
                if not pd.isna(decoy_smiles):
                    decoy_text = str(decoy_smiles).strip()
                if decoy_text:
                    decoy_name = "decoy"
                    if decoy_compound_col:
                        decoy_compound = str(row[decoy_compound_col]).strip()
                        if decoy_compound and decoy_compound != "NAN":
                            decoy_name = decoy_compound
                    _add_pair(
                        pdb_id=pdb_id,
                        ligand=ligand,
                        chain=chain,
