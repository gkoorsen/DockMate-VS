...
from dockmate_vs.gui.unknown_charts import ScoreHover, unknown_docking_data, unknown_score_figure
...
def test_raw_unknown_scores_exclude_controls_and_failures_without_truncation():
    rows = [record(f"sample {i}", best_score=-i) for i in range(77)]
    rows += [
        ...
        record("missing", best_score=None), record("nan", best_score=float("nan")),
        record("infinite", best_score=float("inf")),
    ]
...
def test_groups_preserve_structures_engines_and_missing_score_rows():
    groups = unknown_docking_data([
        ...
        record("unscored receptor", pdb_id="3ABC", best_score=None),
...
def test_figure_plots_every_score_highlights_minimum_and_keeps_ties_hoverable():
    groups = unknown_docking_data([
        record("worse", best_score=-3), record("best", best_score=-9),
        record("tied best", best_score=-9), record("missing", pdb_id="2ABC", best_score=None),
    ])
...
def test_csv_only_saved_results_receive_unknown_chart_data(tmp_path):
    ...
    assert summary["unknown_docking_scores"][0]["points"][0]["score"] == -7


def test_hover_prefers_visible_highlight_over_nearly_overlapping_gray_point():
    groups = unknown_docking_data([
        record("highlighted best", best_score=-8.744),
        record("almost best", best_score=-8.7439), record("other", best_score=-3),
    ])
    figure, axis, points = unknown_score_figure(groups)
    canvas = FigureCanvasAgg(figure)
    canvas.draw()
