...
# Follow a chain of symlinks without relying on readlink -f, which is absent on
# older macOS. Returns the path unchanged when it is not a symlink.
resolve_symlink() {
  local path="$1" target
  while [[ -L "$path" ]]; do
...
if [[ -n "$rdock_root" ]]; then
  ...
  # rbdock may be a symlink into an isolated engine environment. RBT_ROOT must
  # name the environment rDock actually lives in, which holds its data
  # directory, not merely the environment the executable is visible from.
  existing_rbdock="$(resolve_symlink "$CONDA_PREFIX/bin/rbdock")"
  configure_rdock "$(dirname "$(dirname "$existing_rbdock")")"
fi
